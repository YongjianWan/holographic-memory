...
- **A. 检索质量（RQ）**：默认 search 已降为 FTS5 + Jaccard 两路 RRF；HRR 保留给 `probe` / `related` / `reason` 和显式审计。
- **B. 库卫生（P0/P1）**：P0 写入探重、P1-1 entity 归一化、documents 表、soft-delete consolidation、GC/recency 壳已落地；P2 图边已被真实数据否决。

当前稳定快照以 [SESSION.md](SESSION.md) 和最新只读报告为准：`4347 total / 2195 active / 2152 soft-deleted / schema v10`，active category 为 `project 2079 / personal 110 / user_pref 5 / general 1`。旧的 `2200/2147/2078/2071/333/1051/1034` 等数字只作为历史口径，不能再驱动 schema 决策。
...
6. **RQ：HRR 已退出默认 search**
   - `tests/scripts/run_rrf_ab_audit.py` 固定查询 A/B 显示：3-way vs 2-way median top5 overlap 0.8，min 0.4，20 条里 12 条 top1 改变，`hrr_only_top3_query_count=0`。
   - 默认 search 已降到 FTS5 + Jaccard 两路 RRF；HRR 保留给 `probe` / `related` / `reason` 和显式审计路径。
