...
3. **HRR bank 饱和解耦**
   - `project` bank 仍过宽，当前 fact_count 2083，1024 维估算 SNR 约 0.701。
   - `reports/hrr_bank_partition_audit.md` 的只读实测结论：`category_source_doc` 将最大 bank 压到 264（仍略超 256），`category_document_family` 最大 895（不够），`category_source_doc_shard256` 将最大 bank 压到 256、SNR 2.0、无超载。
   - 解法不默认绑定 scope；下一步优先实现 `category + source_doc_id + shard256` 的派生/运行时 bank 方案。若 retrieval 侧证明确实无收益，再承认 HRR 只保留在 probe/reason 实验路径。
