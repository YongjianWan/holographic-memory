...
### HRR 不再作为默认 search 召回/排序腿

2026-06-29 的固定查询 A/B 显示，HRR 没有给默认 search 带来独占 top 召回，却会显著重排 FTS5/Jaccard 的结果。因此 `FactRetriever.search()` 已降为 FTS5 + Jaccard 两路 RRF。

- **当前影响**：默认 search 牺牲 HRR 弱结构信号，换取更可审计、更稳定的关键词候选排序；`probe` / `related` / `reason` 仍使用 HRR。
- **守住边界**：不要为了“语义召回”把 HRR 悄悄塞回默认 search。若未来要恢复，必须先更新固定查询评测集并给出人工 relevance 标签，而不是只看 overlap。
