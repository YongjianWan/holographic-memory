...
过程被当作 fact 写入。历史审计曾识别出 49 条候选；当前 reports 与实时库口径
已经发生变化，具体候选数需要在第 2 步稳定快照审计中重新确认。

- **当前影响**：脏数据污染真实库样本。
- **偿还计划**：
  - **临时治标**：稳定快照后列出 dirty fact 候选，人工复核后通过 `merged_into` 软删除，不物理 DELETE。
...
`add_fact` 每写入一条 fact 都会全量重建对应 category bank。真实批量 retain
时该路径会产生大量重复重建和 SNR 日志，耗时随库规模快速放大。

- **当前影响**：批量 retain 接近 O(n²)。旧审计曾记录 `project` bank 达
  2071 条、1024 维下估算 SNR 约 0.70；当前值需在稳定快照后重算。
- **偿还计划**：为批量 retain 增加事务级延迟重建，整批 facts 完成后每个
  category 只重建一次；在 scope 门通过前不以拆 category 规避问题。

### Scope 可分性尚未通过真实数据门

旧的 go/no-go 和 scope gate 报告之间存在口径冲突，且当前实时库已出现大量
soft-deleted facts。把未经重新核账的 scope 结论焊入 schema，会制造一个长期
不可靠的过滤条件。

- **当前影响**：`project` category 仍可能过宽，但不能用想当然的单值 scope 修复。
- **偿还计划**：先做稳定 DB 快照和只读领域候选审计，统计单领域 / 多领域 /
...
旧只读审计曾按 fact 内容完成多标签初判，但当前报告与实时库口径不一致。这
支持“重新审计 scope”的必要性，而不是批准任何现有 taxonomy。

- **当前影响**：不能据此直接写 migration。
- **偿还计划**：第 2 步基于稳定快照重新生成样本，并复核无匹配、双域、三域以上及
