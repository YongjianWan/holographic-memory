...
过程被当作 fact 写入。历史审计曾识别出 49 条候选；当前稳定快照后仍有少量
meta/dirty candidates 需要人眼确认。

- **当前影响**：脏数据污染真实库样本。
- **偿还计划**：
  - **临时治标**：复核当前快照 dirty fact 候选，确认后通过 `merged_into` 软删除，不物理 DELETE。
...
> **当前活跃债务总览**：L1 **0** | L2 **6** | 架构 **1** | L3 **3** | 合计 **10**
...
*Last updated: 2026-06-26（活跃债务：L1=0 / L2=6 / 架构=1 / L3=3）*
