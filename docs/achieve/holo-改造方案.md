...
这是规格修订，不是实现指令。 它收窄 P1-4 的定义边界，不改任何实现路径——
实现仍是原方案：GC 惰性定时器醒来 → 一次性 API 调通用模型 → 出口闸过滤。没有新组件。
生效门槛（gated，不可跳）：本条目所有内容 悬置于「数据填充 + Gate A go/no-go」之后。
Gate A 未过（翻 50 条 fact，全是「Aiden 干了啥」、抽不出独立于人的结构）→ P1-4 无原料，
规格再清晰也是空中楼阁，一行不写。本条目是「Gate A 过了之后 P1-4 长什么样」的提前定型，
不是「现在可以动手」的信号。
来源：拆 Honcho 文档（Deriver/Dreamer/reasoning 谱系）的副产物。Honcho 给的是规格
（P1-4 该做成什么），不是实现（怎么做出来）——它的实现全踩红线（常驻 Dreamer + 自训模型 +
reasoning tree + surprisal 调度），一行不可抄。本条目只取它的「维度切分」与「判据」。


修订一：P1-4 精确锚定为 只做 induction
Honcho 把 reasoning 拆成四档。P1-4 在其中只占一档，另三档全部显式排除：
档位是什么P1-4凭什么explicit直接陈述的事实归原子 fact 抽取，不归 P1-4这是写路径的事inductive跨多条 fact 认结构模式✓ 这就是 P1-4「Holo极简 / Scout不空转 / 拒Hindsight因常驻 → 共享同一审美」deductive从前提确定推出新断言✗ 砍造新 fact，与「删不加」审美冲突，膨胀风险高abductive从行为模式反推动机✗ 砍（见修订三）撞人格设定「不替用户诊断动机」红线
裁决句（写进 §5 P1-4 描述）：

P1-4 只做 induction（跨领域结构相似性识别），不做 deduction（生成新事实断言），
不做 abduction（揣测动机/心理状态）。


修订二：source_fact_ids 非空闸 —— 把软约束焊成 schema 约束
原 §5 P1-4 第三约束是口头闸：「必须落地为跨领域的结构相似，连不到具体源 fact 的丢弃」。
Honcho 的 reasoning tree 把同一判据做成了数据结构（每条结论物理链到 premises）。抄判据，不抄树。
改动：P1-4 产出的每条 observation 必须带一个 source_fact_ids 字段（fact id 列表）。

入库前校验：source_fact_ids 为空 → 拒绝入库，不存。
这把「禁止漂亮废话」从「靠 LLM 自觉指向源 fact」升级为「schema 层硬拒绝」。
零额外架构：就是 observation 表加一列。不是树，不是图，不需要 get_reasoning_chain 那种遍历。
阈值待标：「几条 source_fact_ids 才算真链得上」是数据上的事，起点设 ≥2（单条 fact 不构成「跨」），
真实数据上验。这个数 Honcho 给不了——它的 proof count 是它数据 + 自训模型调的。

⚠️ 这条改动若涉及 observation 表加列，走真 migration（§7 铁律），不删库。

修订三：禁止动机揣测 —— 比「只 induction」更细的边界
induction 和 abduction 极易滑混，必须显式划开，因为它直接撞人格设定：

induction（要）：描述结构同构，不碰人。
「Holo / Scout / 拒Hindsight 这三个决策共享一个审美：拒绝常驻、拒绝空转」← 陈述结构
abduction（禁）：从模式反推内在状态。
「Aiden 反复要极简 → 他大概是个怕失控 / 控制欲强的人」← 揣测动机

Honcho 把 abduction 当核心卖点（"infer the simplest explanation for behavior"，例：
「saving for house + 抱怨订阅费 → budget-conscious」）。Honcho 当卖点，Holo 当红线。
裁决句（写进 §5 P1-4 描述）：

P1-4 串联产物只陈述「哪几条决策/偏好在结构上同构」，
禁止生成「因此他是一个怎样的人 / 他为什么这样」这类动机或心理推断。
抽结构，不抽人——与「剥皮刀」审美一致。


修订四：提炼粒度的双向参照（非阈值，是参照系）
Honcho 的 token batching（攒到 ~1000 token 再跑一次推理）给了粒度一个输入端参照，
正好和你已有的 输出端 约束（SNR warning：dim=1024 时单条 fact 超 ~256 item 则向量糊）夹成一个区间：

输入端（喂 LLM 时）：别太碎。Honcho 的理由是「<1000 token 没有足够上下文可推」。
→ retain_document chunk 大小起点参考 ~1000 token，不逐句喂、不整篇塞一条。
输出端（提炼成 fact 后）：别太粗。单条 fact 的 content 词数 + entity 数压在 SNR warning 线下。

两端不冲突，是两个层次：输入攒成 ~1000 token 块 → LLM 切成多条细原子 fact。
⚠️ 1000 token 是 Honcho 的数，不是你的终点。和所有外部常数一样：起点参考，真实数据上自己标
（试跑 3-5 个文件时盯 SNR warning + token 成本，§4 既定姿势）。

修订五：质量天花板的诚实声明（不是约束，是预期管理）
Honcho 的 reasoning 跑在自训小模型（Neuromancer XR）上，专门为 formal logic 调过
（follow 逻辑规则、不生成「听起来合理但逻辑不严」的文本）。Holo 用不了——零依赖、一次性 API 调通用模型。
后果，睁眼认：

通用模型做归纳串联，会比自训模型更容易产出「漂亮但逻辑松」的 observation。
Holo 的防线是出口闸（修订二的 source_fact_ids 非空），不是源头模型质量。
所以 P1-4 产物质量天花板低于 Honcho 的 Dreamer，这是无依赖的代价，不是 bug。
别因为产物不如 Honcho 干净，就想给它加自训模型 / embedding —— 那就违红线了。
产物糙到不能用，是「P1-4 这条路在无依赖约束下不成立」的信号，该砍 P1-4，不是该加依赖。


一句话状态
P1-4 规格定型：只 induction、source_fact_ids 非空闸、禁动机揣测、粒度双向参照、天花板诚实认。
实现路径未变（惰性 GC + 一次性 API + 出口闸，原方案既有）。
生效全程 gated on：数据填充 + Gate A go/no-go。Gate A 不过，本条目全部作废——P1-4 没原料。   